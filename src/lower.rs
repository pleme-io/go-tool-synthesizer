...
/// A worlds-separate violation found by the render-time guard: a forbidden
/// substring (the active profile's vendor name) leaked into a GENERATED shell
/// file. The generated shell must stay 100% vendor-free — the only file allowed
/// to name a backend is the hand-written, never-generated `client_adapter.go`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorldsSeparateViolation {
    /// The generated file the forbidden token leaked into (repo-relative).
    pub file: String,
    /// The forbidden substring (the active profile's vendor name).
    pub token: String,
}

impl std::fmt::Display for WorldsSeparateViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "worlds-separate violation: generated shell file `{file}` contains the \
             forbidden token `{token}` (the active profile forbids it). The generated \
             shell must name NO vendor — only the hand-written client_adapter.go may. \
             Hint: spec text (e.g. a command :long, :summary, :usage, or :description) \
             leaked the vendor name into a rendered file; rephrase it to be generic.",
            file = self.file,
            token = self.token,
        )
    }
...
impl std::error::Error for WorldsSeparateViolation {}

/// The forbidden substrings for a resolved borealis profile (case-insensitive
/// match). The `tundra` (private/arctic) profile forbids "akeyless" — the one
/// vendor the engine must never bake into a generated shell. `nord` / the
/// generic default forbid nothing (the public, vendor-free world).
///
/// This is the profile-level `forbidden_substrings` list: a new vendor world is
/// added by extending the match, never by per-emitter string checks.
#[must_use]
pub fn forbidden_substrings(profile: &str) -> &'static [&'static str] {
    if profile.eq_ignore_ascii_case("tundra") {
        &["akeyless"]
    } else {
        &[]
    }
...
/// Lower a [`GoToolSpec`] and ENFORCE the worlds-separate invariant at render
/// time: every generated shell file is scanned (case-insensitively) for the
/// active profile's [`forbidden_substrings`]. The first leak fails the forge
/// with a [`WorldsSeparateViolation`] naming the file + the offending token +
/// the leak hint — so a vendor name that slipped into spec text (a command
/// `:long`, `:summary`, `:usage`, or the tool `:description`) is caught BEFORE
/// the shell is written, not only by a downstream cargo test.
///
/// Every file [`lower`] emits IS a shell file (the hand-written
/// `client_adapter.go` is never produced by the engine), so the whole emitted
/// set is scanned. On success returns the same `(path, GoFile)` set as [`lower`].
pub fn lower_guarded(
    spec: &GoToolSpec,
) -> Result<Vec<(PathBuf, GoFile)>, WorldsSeparateViolation> {
    let files = lower(spec);
    let forbidden = forbidden_substrings(&spec.resolved_profile());
    if !forbidden.is_empty() {
        for (path, file) in &files {
            let rendered = go_synthesizer::print_file(file).to_ascii_lowercase();
            for token in forbidden {
                if rendered.contains(&token.to_ascii_lowercase()) {
                    return Err(WorldsSeparateViolation {
...
fn build_default_config(spec: &GoToolSpec) -> GoFuncDecl {
    // Seed each field from its declared `:default` (typed by the field's scalar
    // kind). A field WITHOUT a `:default` is omitted from the composite, so it
    // takes the Go zero value ("" for Str/Secret, 0 for Int, false for Bool) —
    // NEVER the field name as a placeholder (the historical AuthKind:"AuthKind"
    // / GatewayUrl:"GatewayUrl" bug). DefaultConfig is the lowest-precedence
    // baseline; env + file layers override it before the validator runs.
    let mut composite_fields: Vec<(Option<String>, GoExpr)> = vec![];
    for cf in &spec.config_fields {
        if let Some(default) = cf.default.as_deref() {
            composite_fields.push((
                Some(pascal_case(&cf.name)),
                config_default_expr(&cf.ty, default),
            ));
        }
    }
...
/// The typed Go expression for a config field's declared `:default`, coerced to
/// the field's scalar kind. `Secret` wraps the literal in `shikumi.NewSecret(…)`
/// so the typed `shikumi.Secret[string]` field is well-formed. A non-parseable
/// Int/Bool default falls back to the zero value rather than emitting invalid Go.
fn config_default_expr(ty: &FieldType, default: &str) -> GoExpr {
    match ty {
        FieldType::Str => GoExpr::str(default),
        FieldType::Secret => GoExpr::call(
            GoExpr::path(&["shikumi", "NewSecret"]),
            vec![GoExpr::str(default)],
        ),
        FieldType::Int => default
            .parse::<i64>()
            .map(|n| GoExpr::Lit(go_synthesizer::GoLit::Int(n)))
            .unwrap_or_else(|_| GoExpr::Lit(go_synthesizer::GoLit::Int(0))),
        FieldType::Bool => GoExpr::Lit(go_synthesizer::GoLit::Bool(default == "true")),
    }
}
...
fn build_new(spec: &GoToolSpec) -> GoFuncDecl {
    ...
    // The Action kind wires its GitHub-action composition root (the action/gen
    // entrypoints) ALONGSIDE the spec :commands — both live under the one App, so
    // a forged Action tool needs ZERO hand-edits to main/app to expose its verbs.
...
fn lower_action(spec: &GoToolSpec) -> Vec<(PathBuf, GoFile)> {
    // The Action kind reuses the proven Cli main (load → derive → theme → grammar
    // → execute → exit) so cfg/log/theme are available to BOTH the spec :commands
    // (their api_op dispatch needs cfg for NewClient) and the action/gen
    // composition root. app.New (build_app) adds ActionCommand AND every spec
    // command, so the Action tool exposes all its verbs with NO hand-edit to
    // main.go (the historical ci-secrets regen-safety break).
    let mut files = vec![(PathBuf::from("main.go"), build_main(spec))];
    files.extend(base_app_files(spec));
