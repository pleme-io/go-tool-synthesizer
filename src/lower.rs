...
fn build_serve_fn(spec: &GoToolSpec, wires_server: bool, wires_controller: bool) -> GoFuncDecl {
    ...
    if wires_controller {
        // ctrl, err := controller.New(cfg.Controller, reconciler)
        body.push(GoStmt::Comment(
            "controller-go is declared: build the reconcile chassis from its sub-struct + a\n\
             ReconcileFunc, then run it as a ctx-aware App.Go unit (the controller owns its own\n\
             manager + work queue; lifecycle owns the spine)."
                .into(),
        ));
        body.push(GoStmt::ShortDecl {
...
fn build_daemon_fn(spec: &GoToolSpec, uses_refresh: bool) -> GoFuncDecl {
    ...
    if uses_refresh {
        ...
        if spec.oneshot {
            ...
            // ticker := time.NewTicker(time.Second); defer ticker.Stop() (no defer node — use a loop)
            body.push(GoStmt::ShortDecl {
            ...
            // for { select { case <-ctx.Done(): return nil; case <-ticker.C: log... } } — but
            // the AST has no select/for{}; model as a bounded ForRange over a one-element
            // channel is wrong. Use the simplest honest provable shape: block on ctx.Done().
            body.push(GoStmt::Expr(GoExpr::call(
                GoExpr::sel(GoExpr::ident("ticker"), "Stop"),
                vec![],
            )));
            body.push(GoStmt::Comment(
                "Block until the run context is cancelled (SIGINT/SIGTERM), then return nil for a\n\
                 clean stop. A real daemon drives the ticker in a for/select; the ctx-aware block\n\
                 is the provable minimal shape."
                    .into(),
            ));
            body.push(GoStmt::Expr(GoExpr::Receive(Box::new(GoExpr::call(
                GoExpr::sel(GoExpr::ident("ctx"), "Done"),
...
fn build_client(spec: &GoToolSpec) -> GoFile {
    ...
    f.doc = Some(
        "Client is the abstract API seam: one method per declared api_op, ctx-first, over\n\
         typed request/response placeholders. The generated tool dispatches commands through\n\
         this interface; the CONCRETE client is supplied later by a PRIVATE adapter (the\n\
         absorption layer), so the public engine never bakes in a vendor SDK (worlds-separate)."
            .into(),
    );
